...
// ---- USER ALERTS (in-memory, เพิ่ม DB ทีหลังได้) ----
let userAlerts = [];
// Format: { id, symbol, condition: "above"|"below", price, active: true }
...
5. 🎯 Options Opportunity ที่ดีสุด
...
// ---- AUTO SCAN + TELEGRAM ----
const SCAN_STOCKS = [
  { symbol: "NVDA", sector: "AI/Chip" },
  { symbol: "AAPL", sector: "Tech" },
  { symbol: "TSLA", sector: "EV" },
  { symbol: "RKLB", sector: "Space" },
  { symbol: "MSFT", sector: "Tech" },
  { symbol: "AVGO", sector: "AI/Chip" },
  { symbol: "META", sector: "Tech" },
  { symbol: "ASTS", sector: "Space" },
  { symbol: "PLTR", sector: "AI" },
  { symbol: "COIN", sector: "Crypto" },
];

async function runAutoScan() {
  console.log("🔍 Auto scan running...", new Date().toISOString());
  try {
    ...
    const msg =
      `🤖 <b>AI Market Scan</b> — ${time}\n\n${result}\n\n` +
      `<i>สแกนอัตโนมัติทุก 5 นาที</i>`;
    await sendTelegram(msg);

    // เช็ค price alerts ด้วย
    await checkAlerts();
  } catch (e) {
...
app.post("/api/analyze", async (req, res) => {
  ...
  try {
    ...
    // ส่ง Telegram ด้วยถ้าเชื่อมแล้ว
    if (TELEGRAM_TOKEN) {
      await sendTelegram(
        `🔮 <b>AI วิเคราะห์ ${symbol}</b>\n\n${result}`
      );
    }
...
app.post("/api/scan", async (req, res) => {
  ...
  try {
    ...
    const result = await aiMarketScan(stocks || SCAN_STOCKS);
    scanCache = result;
    scanTime = Date.now();
...
app.post("/api/alerts", async (req, res) => {
  ...
  // แจ้ง Telegram ว่าตั้ง alert แล้ว
  await sendTelegram(
    `🔔 <b>ตั้งแจ้งเตือนสำเร็จ</b>\n\n` +
    `หุ้น: <b>${alert.symbol}</b>\n` +
    `เงื่อนไข: ${condition === "above" ? "ราคาขึ้นถึง" : "ราคาลงถึง"} <b>$${price}</b>`
  );
...
app.get("/api/test-telegram", async (req, res) => {
  const ok = await sendTelegram(
    "🚀 <b>AI Trading Bot เชื่อมต่อสำเร็จ!</b>\n\nระบบพร้อมแจ้งเตือนแล้วครับ ✅"
  );
  res.json({ success: ok, message: ok ? "ส่ง Telegram สำเร็จ!" : "ไม่ได้ตั้งค่า TELEGRAM_TOKEN" });
});
