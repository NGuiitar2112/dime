...
// ====================================================
// ⚙️ แก้บรรทัดนี้หลัง deploy Railway
// ====================================================
...
export default function App() {
  ...
  const handleScan = async () => {
    ...
    try {
      ...
      setScanResult("⚠️ Backend ออฟไลน์");
    }
  ...
  const addAlert = async () => {
    if (!alertSymbol || !alertPrice) {
      showNotif("❌ ใส่ข้อมูลให้ครบ", "#EF4444");
      return;
    }
    try {
      const res = await fetch(`${API_BASE}/api/alerts`, {
      ...
      const d = await res.json();
      if (d.success) {
        showNotif(`✅ ตั้งแจ้งเตือน ${alertSymbol.toUpperCase()} สำเร็จ!`);
        setAlertSymbol("");
        setAlertPrice("");
        loadAlerts();
      }
    } catch {
      showNotif("⚠️ Backend ออฟไลน์", "#EF4444");
    }
  };
  ...
  const deleteAlert = async (id) => {
    try {
      ...
      showNotif("🗑️ ลบแจ้งเตือนแล้ว", "#F59E0B");
      loadAlerts();
    } catch {}
  };
  ...
  const testTelegram = async () => {
    try {
      ...
      const d = await res.json();
      setTelegramStatus(d.success);
      showNotif(d.success ? "✅ ส่ง Telegram สำเร็จ!" : "❌ ยังไม่ได้ตั้งค่า Telegram",
        d.success ? "#10B981" : "#EF4444");
    } catch {
      showNotif("⚠️ Backend ออฟไลน์", "#EF4444");
    }
  };
  ...
  return (
    <div style={{ minHeight:"100vh", maxWidth:480, margin:"0 auto", background:"#080D1A", color:"#E2E8F0", fontFamily:"'Inter',-apple-system,sans-serif" }}>
      ...
      {/* Notif */}
      {notif && (
        <div style={{ position:"fixed", top:16, left:"50%", transform:"translateX(-50%)", background:notif.color, color:"#fff", padding:"10px 20px", borderRadius:12, fontWeight:700, zIndex:9999, fontSize:13, whiteSpace:"nowrap", boxShadow:"0 8px 32px rgba(0,0,0,0.5)" }}>
          {notif.msg}
        </div>
      )}

      {/* Header */}
      <div style={{ padding:"14px 16px 10px", background:"#0D1527", borderBottom:"1px solid rgba(0,212,255,0.1)", position:"sticky", top:0, zIndex:50 }}>
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
          <div style={{ display:"flex", alignItems:"center", gap:8 }}>
            <span style={{ fontSize:20 }}>🚀</span>
            <div>
              <div style={{ fontSize:16, fontWeight:800 }}>AI <span style={{ color:"#00D4FF" }}>Trade</span></div>
              <div style={{ display:"flex", alignItems:"center", gap:4 }}>
                <div style={{ width:6, height:6, borderRadius:"50%", background:backendOnline?"#10B981":"#EF4444" }} />
                <span style={{ fontSize:9, color:"#475569" }}>{backendOnline?"Backend Online":"Backend Offline"}</span>
                {telegramStatus === true && <span style={{ fontSize:9, color:"#10B981" }}>• 📱 Telegram ✅</span>}
              </div>
            </div>
          </div>
          <div onClick={() => setTab("chart")} style={{ background:"rgba(0,212,255,0.08)", border:"1px solid rgba(0,212,255,0.2)", borderRadius:10, padding:"6px 12px", cursor:"pointer" }}>
            <div style={{ fontSize:13, fontWeight:800, color:"#00D4FF" }}>{selected.symbol}</div>
            <div style={{ fontSize:12, fontWeight:700, color:curChange>=0?"#10B981":"#EF4444" }}>
              {curPrice>0?`$${curPrice.toFixed(2)}`:"Loading..."} {curChange!==0&&`${curChange>=0?"▲":"▼"}${Math.abs(curChange).toFixed(2)}%`}
            </div>
          </div>
        </div>
      </div>
      ...
      {/* Tabs */}
      <div style={{ display:"flex", background:"#0D1527", borderBottom:"1px solid rgba(255,255,255,0.05)", position:"sticky", top:66, zIndex:40, overflowX:"auto" }}>
        {TABS.map(t => (
          <button key={t.id} onClick={() => setTab(t.id)} style={{ flex:1, minWidth:56, padding:"11px 3px", background:"transparent", border:"none", borderBottom:tab===t.id?"2px solid #00D4FF":"2px solid transparent", color:tab===t.id?"#00D4FF":"#475569", fontSize:10, fontWeight:tab===t.id?700:400, cursor:"pointer", whiteSpace:"nowrap" }}>
            {t.label}
          </button>
        ))}
      </div>
      ...
      {tab==="market" && (
        <div style={{ padding:"12px 12px 80px" }}>
            ...
            <div style={{ marginBottom:14 }}>
              ...
              <div style={{ display:"flex", gap:8, overflowX:"auto", paddingBottom:4 }}>
                {watchlist.map(sym => {
                  const p = prices[sym];
                  return (
                    <div key={sym} onClick={()=>{ const s=STOCKS.find(x=>x.symbol===sym); if(s){setSelected(s);setTab("chart");}}}
                      style={{ minWidth:85, background:"rgba(0,212,255,0.06)", border:"1px solid rgba(0,212,255,0.15)", borderRadius:10, padding:"8px 10px", cursor:"pointer" }}>
                      <div style={{ fontSize:12, fontWeight:800, color:"#00D4FF" }}>{sym}</div>
                      <div style={{ fontSize:13, fontWeight:700 }}>{p?`$${p.price.toFixed(2)}`:"—"}</div>
                      <div style={{ fontSize:10, fontWeight:600, color:(p?.changePct||0)>=0?"#10B981":"#EF4444" }}>
          ...
          <div style={{ fontSize:10, color:"#475569", fontWeight:600, marginBottom:8, letterSpacing:0.5 }}>{filtered.length} หุ้น • อัพเดทราคาทุก 60 วิ</div>
          {filtered.map(stock => {
            const p = prices[stock.symbol];
            const isSelected = selected.symbol===stock.symbol;
            return (
              <div key={stock.symbol} onClick={()=>{setSelected(stock);setTab("chart");}}
                style={{ display:"flex", alignItems:"center", padding:"11px 12px", marginBottom:5, background:isSelected?"rgba(0,212,255,0.06)":"rgba(255,255,255,0.02)", border:isSelected?"1px solid rgba(0,212,255,0.2)":"1px solid rgba(255,255,255,0.05)", borderRadius:12, cursor:"pointer" }}>
                <div style={{ flex:1 }}>
      ...
      {tab==="chart" && (
        <div style={{ display:"flex", flexDirection:"column", height:"calc(100vh - 120px)" }}>
          <div style={{ padding:"10px 14px", background:"#0D1527", borderBottom:"1px solid rgba(255,255,255,0.05)" }}>
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
              <div>
                ...
                <div style={{ display:"flex", gap:8, marginTop:2 }}>
                  <span style={{ fontSize:18, fontWeight:800 }}>{curPrice>0?`$${curPrice.toFixed(2)}`:"—"}</span>
                  <span style={{ fontSize:13, fontWeight:700, alignSelf:"center", color:curChange>=0?"#10B981":"#EF4444" }}>{curChange>=0?"▲":"▼"}{Math.abs(curChange).toFixed(2)}%</span>
                </div>
                {prices[selected.symbol] && (
                  <div style={{ fontSize:10, color:"#475569", marginTop:2 }}>
                    H:${prices[selected.symbol].high?.toFixed(2)} • L:${prices[selected.symbol].low?.toFixed(2)} • O:${prices[selected.symbol].open?.toFixed(2)}
                  </div>
                )}
              </div>
              <div style={{ display:"flex", gap:5 }}>
                <button onClick={()=>toggleWL(selected.symbol)} style={{ padding:"7px 10px", borderRadius:8, background:watchlist.includes(selected.symbol)?"rgba(245,158,11,0.15)":"rgba(255,255,255,0.04)", border:watchlist.includes(selected.symbol)?"1px solid rgba(245,158,11,0.4)":"1px solid rgba(255,255,255,0.06)", color:watchlist.includes(selected.symbol)?"#F59E0B":"#64748B", fontSize:12, cursor:"pointer" }}>⭐</button>
                <button onClick={()=>handleAnalyze()} style={{ padding:"7px 10px", borderRadius:8, background:"rgba(168,85,247,0.12)", border:"1px solid rgba(168,85,247,0.3)", color:"#A855F7", fontSize:11, fontWeight:700, cursor:"pointer" }}>🤖</button>
                <button onClick={()=>{fetchNews(selected.symbol);setTab("news");}} style={{ padding:"7px 10px", borderRadius:8, background:"rgba(0,212,255,0.08)", border:"1px solid rgba(0,212,255,0.2)", color:"#00D4FF", fontSize:11, fontWeight:700, cursor:"pointer" }}>📰</button>
                <button onClick={()=>{ setAlertSymbol(selected.symbol); setTab("alerts"); }} style={{ padding:"7px 10px", borderRadius:8, background:"rgba(16,185,129,0.08)", border:"1px solid rgba(16,185,129,0.2)", color:"#10B981", fontSize:11, fontWeight:700, cursor:"pointer" }}>🔔</button>
              </div>
            </div>
          </div>
          <div style={{ flex:1, minHeight:0 }}><TradingViewChart symbol={selected.symbol} /></div>
          <div style={{ padding:"8px 10px", background:"#0D1527", borderTop:"1px solid rgba(255,255,255,0.05)", display:"flex", gap:6, overflowX:"auto" }}>
            {["RKLB","NVDA","AAPL","TSLA","AVGO","META","PLTR","COIN"].map(sym => {
              const s = STOCKS.find(x=>x.symbol===sym);
              return (
                <button key={sym} onClick={()=>s&&setSelected(s)} style={{ padding:"6px 10px", borderRadius:8, whiteSpace:"nowrap", background:selected.symbol===sym?"rgba(0,212,255,0.12)":"rgba(255,255,255,0.04)", border:selected.symbol===sym?"1px solid #00D4FF":"1px solid rgba(255,255,255,0.06)", color:selected.symbol===sym?"#00D4FF":"#64748B", fontSize:11, fontWeight:700, cursor:"pointer" }}>{sym}</button>
              );
            })}
          </div>
        </div>
      ...
      {tab==="ai" && (
        <div style={{ padding:"12px 12px 80px" }}>
          <div style={{ background:autoScan?"rgba(16,185,129,0.07)":"rgba(255,255,255,0.03)", border:`1px solid ${autoScan?"rgba(16,185,129,0.25)":"rgba(255,255,255,0.07)"}`, borderRadius:12, padding:"12px 14px", marginBottom:12, display:"flex", justifyContent:"space-between", alignItems:"center" }}>
            <div>
              <div style={{ fontSize:13, fontWeight:700, color:autoScan?"#10B981":"#94A3B8" }}>{autoScan?"🟢 AI Agent 24/7 ทำงานอยู่":"⚫ AI Agent หยุดอยู่"}</div>
              <div style={{ fontSize:10, color:"#475569", marginTop:2 }}>{lastScan?`สแกนล่าสุด ${lastScan}`:"ยังไม่ได้สแกน"}{autoScan&&" • ทุก 5 นาที"}</div>
            </div>
            <button onClick={()=>setAutoScan(!autoScan)} style={{ padding:"8px 12px", borderRadius:10, background:autoScan?"rgba(239,68,68,0.12)":"rgba(16,185,129,0.12)", border:autoScan?"1px solid rgba(239,68,68,0.3)":"1px solid rgba(16,185,129,0.3)", color:autoScan?"#EF4444":"#10B981", fontSize:12, fontWeight:700, cursor:"pointer" }}>{autoScan?"หยุด":"เปิด Auto"}</button>
          </div>
          <button onClick={handleScan} disabled={scanLoading} style={{ width:"100%", padding:13, marginBottom:10, background:scanLoading?"rgba(100,116,139,0.15)":"linear-gradient(135deg,#0f2744,#0a1a30)", border:"1px solid rgba(0,212,255,0.25)", borderRadius:12, color:scanLoading?"#64748B":"#00D4FF", fontSize:14, fontWeight:700, cursor:scanLoading?"not-allowed":"pointer" }}>
            {scanLoading?"⏳ กำลังสแกน...":"🔍 สแกนตลาดทั้งหมด (Real Data)"}
          </button>
          {scanResult && (
            <div style={{ background:"rgba(0,212,255,0.04)", border:"1px solid rgba(0,212,255,0.12)", borderRadius:12, padding:14, marginBottom:12 }}>
              <div style={{ fontSize:11, color:"#00D4FF", fontWeight:700, marginBottom:8 }}>📊 ผลสแกน {lastScan&&`• ${lastScan}`}</div>
              <div style={{ fontSize:13, color:"#CBD5E1", lineHeight:1.9, whiteSpace:"pre-wrap" }}>{scanResult}</div>
            </div>
          )}
          <button onClick={()=>handleAnalyze()} disabled={aiLoading} style={{ width:"100%", padding:13, marginBottom:10, background:aiLoading?"rgba(100,116,139,0.15)":"linear-gradient(135deg,#1a0a3d,#0d0624)", border:"1px solid rgba(168,85,247,0.25)", borderRadius:12, color:aiLoading?"#64748B":"#A855F7", fontSize:14, fontWeight:700, cursor:aiLoading?"not-allowed":"pointer" }}>
            {aiLoading?`⏳ วิเคราะห์ ${selected.symbol}...`:`🔮 วิเคราะห์ ${selected.symbol} (ราคา+ข่าวจริง)`}
          </button>
          {aiResult && (
            <div style={{ background:"rgba(168,85,247,0.04)", border:"1px solid rgba(168,85,247,0.12)", borderRadius:12, padding:14, marginBottom:12 }}>
              ...
              <div style={{ fontSize:13, color:"#CBD5E1", lineHeight:1.9, whiteSpace:"pre-wrap" }}>{aiResult}</div>
            </div>
          )}
          ...
          <div style={{ display:"flex", flexWrap:"wrap", gap:7 }}>
            {["RKLB","NVDA","AAPL","TSLA","AVGO","META","PLTR"].map(sym=>(
              <button key={sym} onClick={()=>{ const s=STOCKS.find(x=>x.symbol===sym); if(s){setSelected(s);handleAnalyze(s);}}} style={{ padding:"8px 12px", borderRadius:10, background:"rgba(255,255,255,0.04)", border:"1px solid rgba(255,255,255,0.07)", color:"#94A3B8", fontSize:12, fontWeight:600, cursor:"pointer" }}>🤖 {sym}</button>
            ))}
          </div>
        </div>
      )}
        ...
        <div style={{ padding:"12px 12px 80px" }}>
          ...
          <div style={{ display:"flex", gap:6, overflowX:"auto", marginBottom:14, paddingBottom:4 }}>
            {["RKLB","NVDA","AAPL","TSLA","AVGO","META","MSFT"].map(sym=>(
              <button key={sym} onClick={()=>{ const s=STOCKS.find(x=>x.symbol===sym); if(s){setSelected(s);fetchNews(sym);}}} style={{ padding:"6px 10px", borderRadius:8, whiteSpace:"nowrap", background:selected.symbol===sym?"rgba(0,212,255,0.12)":"rgba(255,255,255,0.04)", border:selected.symbol===sym?"1px solid #00D4FF":"1px solid rgba(255,255,255,0.06)", color:selected.symbol===sym?"#00D4FF":"#64748B", fontSize:11, fontWeight:700, cursor:"pointer" }}>{sym}</button>
            ))}
          </div>
          {newsLoading && <div style={{ textAlign:"center", padding:"40px 0", color:"#475569" }}><div style={{ fontSize:24 }}>⏳</div><div style={{ fontSize:13, marginTop:8 }}>กำลังโหลด...</div></div>}
          ...
          {news.map((item,i)=>(
            <div key={i} style={{ background:"rgba(255,255,255,0.03)", border:"1px solid rgba(255,255,255,0.06)", borderRadius:12, padding:14, marginBottom:10 }}>
              <div style={{ fontSize:13, fontWeight:600, color:"#E2E8F0", lineHeight:1.5, marginBottom:6 }}>{item.headline}</div>
      ...
      {tab==="alerts" && (
        <div style={{ padding:"12px 12px 80px" }}>
          ...
          <div style={{ background:"rgba(255,255,255,0.03)", border:"1px solid rgba(255,255,255,0.07)", borderRadius:12, padding:"12px 14px", marginBottom:14 }}>
            <div style={{ fontSize:12, fontWeight:700, marginBottom:10 }}>📱 Telegram Bot</div>
            <div style={{ fontSize:11, color:"#64748B", marginBottom:10, lineHeight:1.7 }}>
              การแจ้งเตือนจะส่งผ่าน Telegram Bot อัตโนมัติ ต้องตั้งค่า{" "}
              <span style={{ color:"#F59E0B", fontWeight:700 }}>TELEGRAM_TOKEN</span> และ{" "}
              <span style={{ color:"#F59E0B", fontWeight:700 }}>TELEGRAM_CHAT_ID</span>{" "}
              ใน Railway Variables
            </div>
            <button onClick={testTelegram} style={{ width:"100%", padding:"10px", borderRadius:10, background:"rgba(0,212,255,0.08)", border:"1px solid rgba(0,212,255,0.2)", color:"#00D4FF", fontSize:13, fontWeight:700, cursor:"pointer" }}>
              🔔 ทดสอบส่ง Telegram
            </button>
          </div>
          ...
          <div style={{ background:"rgba(255,255,255,0.03)", border:"1px solid rgba(255,255,255,0.07)", borderRadius:12, padding:"14px", marginBottom:14 }}>
            <div style={{ fontSize:12, fontWeight:700, marginBottom:12 }}>➕ ตั้งแจ้งเตือนใหม่</div>
            <div style={{ marginBottom:10 }}>
              <div style={{ fontSize:10, color:"#475569", marginBottom:5, fontWeight:600 }}>หุ้น</div>
              <input
                value={alertSymbol}
                onChange={e=>setAlertSymbol(e.target.value.toUpperCase())}
                placeholder="เช่น NVDA, AAPL..."
                style={{ width:"100%", padding:"10px 12px", background:"rgba(255,255,255,0.05)", border:"1px solid rgba(255,255,255,0.1)", borderRadius:10, color:"#E2E8F0", fontSize:14, outline:"none" }}
              />
            </div>
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:8, marginBottom:10 }}>
              <div>
                <div style={{ fontSize:10, color:"#475569", marginBottom:5, fontWeight:600 }}>เงื่อนไข</div>
              ...
              <div>
                <div style={{ fontSize:10, color:"#475569", marginBottom:5, fontWeight:600 }}>ราคา ($)</div>
                <input
                  type="number"
                  value={alertPrice}
                  onChange={e=>setAlertPrice(e.target.value)}
                  placeholder="เช่น 150.00"
                  style={{ width:"100%", padding:"10px 12px", background:"rgba(255,255,255,0.05)", border:"1px solid rgba(255,255,255,0.1)", borderRadius:9, color:"#E2E8F0", fontSize:14, outline:"none" }}
                />
              </div>
            </div>
            <button onClick={addAlert} style={{ width:"100%", padding:"12px", borderRadius:10, background:"linear-gradient(135deg,#10B981,#059669)", border:"none", color:"#fff", fontSize:14, fontWeight:800, cursor:"pointer" }}>
              🔔 ตั้งแจ้งเตือน
            </button>
          </div>
          ...
          <div style={{ fontSize:12, fontWeight:700, marginBottom:10 }}>
            แจ้งเตือนที่ตั้งไว้ ({alerts.length})
          </div>
          {alerts.length===0 ? (
            <div style={{ textAlign:"center", padding:"30px 0", color:"#475569" }}>
              <div style={{ fontSize:28 }}>🔕</div>
              <div style={{ fontSize:12, marginTop:8 }}>ยังไม่มีแจ้งเตือน</div>
            </div>
          ) : alerts.map(alert=>(
            <div key={alert.id} style={{ background:"rgba(255,255,255,0.03)", border:"1px solid rgba(255,255,255,0.07)", borderRadius:12, padding:"12px 14px", marginBottom:8, display:"flex", justifyContent:"space-between", alignItems:"center" }}>
              <div>
                ...
                <div style={{ fontSize:10, color:"#475569", marginTop:4 }}>
                  ตั้งเมื่อ {new Date(alert.createdAt).toLocaleString("th-TH")}
                </div>
              </div>
              <button onClick={()=>deleteAlert(alert.id)} style={{ width:32, height:32, borderRadius:8, background:"rgba(239,68,68,0.1)", border:"1px solid rgba(239,68,68,0.2)", color:"#EF4444", fontSize:14, cursor:"pointer" }}>🗑️</button>
            </div>
          ))}
        </div>
      )}
      ...
      {tab==="options" && (
        <div style={{ padding:"12px 12px 80px" }}>
          ...
          <div style={{ marginBottom:12 }}>
            ...
            <div style={{ display:"flex", gap:6, overflowX:"auto", paddingBottom:4 }}>
              {["RKLB","NVDA","AAPL","TSLA","AVGO","META"].map(sym=>{
                ...
                const s=STOCKS.find(x=>x.symbol===sym);
                return (
                  <button key={sym} onClick={()=>s&&setSelected(s)} style={{ padding:"7px 10px", borderRadius:9, whiteSpace:"nowrap", background:selected.symbol===sym?"rgba(0,212,255,0.1)":"rgba(255,255,255,0.04)", border:selected.symbol===sym?"1px solid #00D4FF":"1px solid rgba(255,255,255,0.06)", color:selected.symbol===sym?"#00D4FF":"#64748B", fontSize:11, fontWeight:700, cursor:"pointer" }}>
                    {sym}<br/><span style={{ fontSize:10, fontWeight:400 }}>{p?`$${p.toFixed(0)}`:"—"}</span>
                  </button>
                );
              })}
            </div>
          ...
          <div style={{ marginBottom:12 }}>
            ...
            <div style={{ fontSize:10, color:"#475569", marginTop:3 }}>ราคาปัจจุบัน: ${curPrice.toFixed(2)}</div>
          </div>
          ...
          <div style={{ background:"rgba(0,212,255,0.03)", border:"1px solid rgba(0,212,255,0.1)", borderRadius:12, padding:12, marginBottom:12 }}>
            ...
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
              ...
              <span style={{ fontSize:18, fontWeight:800, color:pnl>=0?"#10B981":"#EF4444" }}>{pnl>=0?"+":""}${pnl.toFixed(2)}</span>
            </div>
          </div>
          <button onClick={()=>{
            if(totalCost>optBal){showNotif("❌ เงินไม่พอ!","#EF4444");return;}
            setPositions([...positions,{id:Date.now(),stock:selected.symbol,type:optType,strike,premium,contracts,totalCost}]);
            setOptBal(b=>Math.round((b-totalCost)*100)/100);
            showNotif(`✅ ซื้อ ${optType.toUpperCase()} ${selected.symbol}`);
          }} disabled={totalCost>optBal} style={{ width:"100%", padding:14, borderRadius:12, border:"none", background:totalCost>optBal?"rgba(100,116,139,0.2)":optType==="call"?"linear-gradient(135deg,#10B981,#059669)":"linear-gradient(135deg,#EF4444,#DC2626)", color:"#fff", fontSize:15, fontWeight:800, marginBottom:12, cursor:totalCost>optBal?"not-allowed":"pointer" }}>
            ...
            return (
              <div key={pos.id} style={{ background:"rgba(255,255,255,0.03)", border:`1px solid ${posPnL>=0?"rgba(16,185,129,0.2)":"rgba(239,68,68,0.2)"}`, borderRadius:12, padding:12, marginBottom:8, display:"flex", justifyContent:"space-between", alignItems:"center" }}>
                ...
                <div style={{ textAlign:"right" }}>
                  <div style={{ fontSize:15, fontWeight:800, color:posPnL>=0?"#10B981":"#EF4444" }}>{posPnL>=0?"+":""}${posPnL.toFixed(2)}</div>
                  <button onClick={()=>{
                    setOptBal(b=>Math.round((b+Math.max(0,pos.totalCost+posPnL))*100)/100);
                    setPositions(positions.filter(p=>p.id!==pos.id));
                    showNotif(posPnL>=0?`🎉 กำไร +$${posPnL.toFixed(2)}`:`📉 -$${Math.abs(posPnL).toFixed(2)}`,posPnL>=0?"#10B981":"#EF4444");
                  }} style={{ marginTop:4, padding:"3px 8px", background:"rgba(239,68,68,0.1)", border:"1px solid rgba(239,68,68,0.2)", borderRadius:6, color:"#EF4444", fontSize:10, cursor:"pointer" }}>ปิด</button>
                </div>
              </div>
            );
          })}
